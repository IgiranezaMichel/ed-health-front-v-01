...
export const CreateCertificate=(props:{trainingId:number})=>{
    ...
    const handleStamp = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            reader.onload = () => {
                setCertificate({ ...certificate, stamp: reader.result as string })
            };
    ...
    const handleSignature = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            const reader = new FileReader();
            reader.onload = () => {
                setCertificate({ ...certificate,signature: reader.result as string })
            };
            reader.readAsDataURL(file);
        }
    }
    return(
       <>
       {
            ...
            <div className="d-flex justify-content-center my-4">
                ...
                <div className="mx-5">
                    <div onClick={handleStampClick} style={{ border: '2px solid blue', borderStyle: 'dashed',width:'100%'}}>
                        {certificate.stamp==''?<div className="text-center" >
                            Add Stamp
                        </div>:<img width={100} src={certificate.stamp}/>}
                    </div>
                ...
                <div onClick={handleSignatureClick} style={{ border: '2px solid blue', borderStyle: 'dashed',width:'100%' }}>
                        {certificate.signature==''?<div className="text-center" style={{ border: '2px solid blue', borderStyle: 'dashed' }}>
                            Add signature
                        </div>:<img width={100} style={{objectFit:'contain'}} src={certificate.signature}/>}
                    </div>
        ...
        <div style={{ width: '40px', height: '40px', float: 'right',}}>
            <QrCode certificateId={1} style={{ width: '100%', height: '100%' }} />
        </div>
        <div className="text-center" style={{clear:'both' }}>
                <CheckBox className="fs-2"/> <CancelPresentation className="fs-2"/>
            </div>
    </Card>
       }
       </>

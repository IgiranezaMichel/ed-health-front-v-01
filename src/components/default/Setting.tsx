...
export const Setting = () => {
    ...
    const imgHandler = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const updateUserHandler=()=>{
        updateUser({variables:{input:userData}}).then(data=>
            {localStorage.removeItem("userData");localStorage.setItem("userData",JSON.stringify(user));alert(data.data.registerUser.name+" Saved successful")}
            ).catch(err=>err);
